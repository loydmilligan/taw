...
export function buildModeSystemPrompt(
  ...
  if (mode === 'Brainstorm') {
    return [
      ...
      '- VOICE SUMMARY: Begin each response with a single sentence prefixed exactly `[VOICE SUMMARY]:` that summarizes your key point or question (e.g. "[VOICE SUMMARY]: I want to understand your core constraint."). This summary line appears on its own, as the first line, before any elaboration.',
      '',
  ...
  if (mode === 'Brainstorm Phase 2') {
    ...
    return [
      ...
      '- VOICE SUMMARY: Begin each response with a single sentence prefixed exactly `[VOICE SUMMARY]:` that summarizes the most important mapping question or update (e.g. "[VOICE SUMMARY]: Here\'s what the map looks like so far."). This summary line appears on its own, as the first line, before the map.',
      '',
      'REQUIRED: End every response with a footer on its own line, formatted exactly like this:',
      '─────────────────────────────────────────',
