...
export async function loadConfig(cwd: string): Promise<LoadedConfig> {
  ...
  const model =
    projectConfig?.model ?? globalConfig.defaultModel ?? getDefaultModelForProvider(provider);
...
export async function saveGlobalConfig(config: GlobalConfig): Promise<void> {
  ...
  await writeFile(configPath, `${JSON.stringify(globalConfigSchema.parse(config), null, 2)}\n`, 'utf8');
}

export async function saveProjectConfig(cwd: string, config: ProjectConfig): Promise<void> {
  const configPath = getProjectConfigPath(cwd);
  await mkdir(path.dirname(configPath), { recursive: true });
  await writeFile(configPath, `${JSON.stringify(projectConfigSchema.parse(config), null, 2)}\n`, 'utf8');
}
...
function getEnvApiKey(provider: ProviderConfig['provider']): string | undefined {
...
function getDefaultBaseUrl(provider: ProviderConfig['provider']): string | undefined {
  if (provider === 'openrouter') {
...
function getDefaultModelForProvider(provider: ProviderConfig['provider']): string {
  if (provider === 'openai') {
    return 'gpt-4o-mini';
  }
